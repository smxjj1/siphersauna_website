  ...
  <div class="news-page">
    <section class="hero-section">
      <div class="hero-container">
        <h1 class="hero-title">{{ tm('news.title') }}</h1>
        <p class="hero-subtitle">{{ tm('news.subtitle') }}</p>
      </div>
    </section>
...
const { tm, t, locale } = useI18nHelpers()
const { fetchNews, recordNewsView } = useNews()
const { trackNewsView } = useAnalytics()

useHead({
  title: computed(() => tm('news.metaTitle')),
  htmlAttrs: { lang: locale },
  meta: [
    { name: 'description', content: computed(() => tm('news.metaDescription')) },
  ],
})
...
function formatDate(value: string) {
  ...
  if (Number.isNaN(d.getTime()))
    return ''
  const localeTag = locale.value === 'zh-CN' ? 'zh-CN' : locale.value === 'zh-TW' ? 'zh-TW' : 'en-US'
